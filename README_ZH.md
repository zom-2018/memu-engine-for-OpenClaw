...
- Vendored 的 MemU 运行时版本：`v1.5.1`
- Python 运行时：`3.13+`
- 原生 OpenClaw 插件会先由 `openclaw.plugin.json` 做校验，再加载运行时代码
- 新插件代码应遵循当前 OpenClaw SDK 的分层子路径导入约定
...
- Python `3.13+`（vendored 的 `memU v1.5.1` 运行时要求）

> 首次运行自动自举：插件会在 `python/.venv` 下创建/复用独立环境，并按 `python/uv.lock` 安装锁定依赖。npm 安装与 GitHub 源码安装都走同一逻辑。
>
> OpenClaw 会在加载运行时代码之前验证插件 manifest，因此 `openclaw.plugin.json` 需要与实际配置面保持同步。
...
**完整 SecretRef 对象**（等同于 `${VAR}` 语法，也是当前 OpenClaw SecretRef 形状的一部分）：
```jsonc
"apiKey": {
  "source": "env",
  "provider": "default",
...
**注意：** 这个插件当前会解析 `env` 型 SecretRef 和 `${VAR}` 模板；`file` 和 `exec` 是更宽的 OpenClaw SecretRef 约定的一部分，但这个插件还不会解析它们。
...
除了记忆布局相关配置之外，manifest 和运行时还支持几个直接的插件级键：

* `dataDir`：覆盖运行时数据根目录，默认 `~/.openclaw/memUdata`
* `userId`：设置 Python worker 使用的记忆命名空间，默认 `default`
* `flushOnCompaction`：启用 `after_compaction` 刷新钩子
* `debugTiming`：开启搜索辅助脚本的额外计时输出
